...
interface ExperientialModalProps {
  onClose: () => void;
  onSave: (data: CalendarData) => void;
  initialData?: CalendarData;
}

const ExperientialModal: FC<ExperientialModalProps> = ({ onClose, onSave, initialData }) => {
  ...
  const fetchEvents = useCallback(async (country: string) => {
    ...
    try {
      ...
      console.error("Failed to fetch experiential events", err);
    } finally {
        ...
        <div className="cardhost open w-full h-full flex items-center justify-center">
          <div className="card">
            <div className="cardtop">
              <h3>🎯 Experiential Event</h3>
            </div>
            <div className="sub">Choose a country from your markets, then select an experiential event.</div>
            ...
            <div style={{ marginBottom: "8px" }}>
              ...
              {primaryMarkets.length > 0 ? (
                <div className="country-buttons">
                  {primaryMarkets.map((country:any) => (
                    <button
                      key={country}
            ...
            {selectedCountry && (
              <div className="selected-country-label">
                ...
                {events.length > 0 && !loadingEvents && (
                  <span className="event-count">({events.length} experiential events available)</span>
                )}
              </div>
            )}
            ...
            {selectedCountry ? (
              loadingEvents ? (
                <div style={{ textAlign: "center", padding: "1rem", color: "#8ea0cc" }}>
                  <span className="spin" /> Loading experiential events for {selectedCountry}...
                </div>
              ) : (
                <div
                  className="chips"
                ...
                >
                  ...
                  {events.length === 0 && !loadingEvents && (
                    <div style={{ color: "#8ea0cc", padding: "12px", width: "100%", textAlign: "center" }}>
                      No experiential events found for {selectedCountry}
                    </div>
                  )}
                </div>
              )
              ...
              <div style={{ color: "#8ea0cc", padding: "20px", textAlign: "center" }}>
                Please select a country above to see experiential events
              </div>
            )}
...
export default ExperientialModal;