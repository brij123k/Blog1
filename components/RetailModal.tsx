...
const RetailModal: FC<RetailModalProps> = ({ onClose, onSave, initialData }) => {
  ...
  const fetchEvents = useCallback(async (country: string) => {
    ...
    try {
      ...
      console.error("Failed to fetch retail events", err);
    } finally {
        ...
        <div className="cardhost open w-full h-full flex items-center justify-center">
          <div className="card">
            <div className="cardtop">
              <h3>🛍️ Retail Event</h3>
            </div>
            <div className="sub">Choose a country from your markets, then select a retail event.</div>
            ...
            {selectedCountry && (
              <div className="selected-country-label">
                ...
                {events.length > 0 && !loadingEvents && (
                  <span className="event-count">({events.length} retail events available)</span>
                )}
              </div>
            )}
            ...
            {selectedCountry ? (
              loadingEvents ? (
                <div style={{ textAlign: "center", padding: "1rem", color: "#8ea0cc" }}>
                  <span className="spin" /> Loading retail events for {selectedCountry}...
                </div>
              ) : (
                <div
                  className="chips"
                ...
                >
                  ...
                  {events.length === 0 && !loadingEvents && (
                    <div style={{ color: "#8ea0cc", padding: "12px", width: "100%", textAlign: "center" }}>
                      No retail events found for {selectedCountry}
                    </div>
                  )}
                </div>
              )
              ...
              <div style={{ color: "#8ea0cc", padding: "20px", textAlign: "center" }}>
                Please select a country above to see retail events
              </div>
            )}
